#ifndef MIG_CHOICE_COMPUTE_HPP
#define MIG_CHOICE_COMPUTE_HPP
#pragma once
...
namespace also
{
struct mig_choice_compute_params
{
  mig_choice_compute_params()
  {
...
struct mig_choice_compute_stats
{
};
namespace detail
{
struct mig_cut_enumeration_choice_compute_cut
{
  int32_t gain{ -1 };
};
template<class Ntk, class RewritingFn, class Library, class RefactoringFn,
         class RefactoringFn2, class NodeCostFn>
class mig_choice_compute_manager
{
public:
  using node = typename Ntk::node;
  using signal = typename Ntk::signal;
  static constexpr node MIG_NULL = mig_network::MIG_NULL;
  static constexpr uint32_t num_vars = 4u;

#define MIG_SIGNAL_NULL ( signal( MIG_NULL, 0 ) )
  mig_choice_compute_manager( Ntk& ntk, RewritingFn&& rewriting_fn,
                              Library&& library, RefactoringFn&& refactoring_fn,
  ...
  {
    const auto cuts =
        cut_enumeration<Ntk, true, mig_cut_enumeration_choice_compute_cut>(
...
private:
  ...
  Ntk derive_choice_mig()
  {
    ntk.clear_values();
    ntk.init_choices( ntk.size() );
    Ntk choice_mig;
    choice_mig.clear_values();
    choice_mig.init_choices( ntk.size() );
    std::vector<signal> old2new( ntk.size(), MIG_SIGNAL_NULL );
    old2new[0] = ntk.get_constant( false );
    ntk.foreach_pi( [&]( node const& i )
                    {
      choice_mig.create_pi();
      old2new[i] = signal(i, 0); } );
    auto get_repr_signal = [&]( signal const& s )
    {
      auto repr = choice_mig.get_repr( s.index );
      return signal( repr, choice_mig.phase( repr ) ^ choice_mig.phase( s.index ) ^
                               s.complement );
    };
    ntk.foreach_gate( [&]( node const& n )
      ...
      auto old_child2 = ntk.get_child2(n);
      ...
      auto new_child2 = old2new[old_child2.index] ^ old_child2.complement;
      auto new_node = choice_mig.create_maj(get_repr_signal(new_child0),
                                            get_repr_signal(new_child1),
                                            get_repr_signal(new_child2));
      while (true) {
        auto new2 = new_node;
        new_node = get_repr_signal(new2);
        if (new_node == new2) break;
      }
      assert(old2new[n] == MIG_SIGNAL_NULL);
      old2new[n] = new_node;
      ...
      if (choice_mig.fanout_size(new_node.index) != 0) return;
      choice_mig.set_choice(new_node.index, new_repr.index); } );
    ntk.foreach_co( [&]( signal const& o )
                    {
      auto new_signal = old2new[o.index] ^ o.complement;
      auto new_repr = choice_mig.get_repr(new_signal.index);
      choice_mig.create_po(signal(new_repr, choice_mig.phase(new_signal.index) ^
                                                choice_mig.phase(new_repr) ^
                                                new_signal.complement)); } );
    return mig_man_dup_dfs( choice_mig );
  }

  Ntk mig_man_dup_dfs( Ntk const& tmp )
  {
    Ntk new_ntk;
    new_ntk.clear_values();
    new_ntk.init_choices( tmp.size() );
    std::vector<signal> old2new( tmp.size(), MIG_SIGNAL_NULL );
    ...
    tmp.foreach_co( [&]( signal const& o )
                    {
      mig_man_dup_dfs_rec(tmp, new_ntk, old2new, o.index);
      new_ntk.create_po(old2new[o.index] ^ o.complement); } );
    return new_ntk;
  }

  static signal mig_man_dup_dfs_rec( Ntk const& tmp, Ntk& new_ntk,
                                     std::vector<signal>& old2new,
                                     node const& n )
  {
    if ( old2new[n] != MIG_SIGNAL_NULL )
      return old2new[n];
    signal new_equiv = signal( MIG_NULL, 0 );
    if ( tmp.get_equiv_node( n ) != MIG_NULL )
    {
      new_equiv =
          mig_man_dup_dfs_rec( tmp, new_ntk, old2new, tmp.get_equiv_node( n ) );
    }
    auto old_child0 = tmp.get_child0( n );
    auto old_child1 = tmp.get_child1( n );
    auto old_child2 = tmp.get_child2( n );
    mig_man_dup_dfs_rec( tmp, new_ntk, old2new, old_child0.index );
    mig_man_dup_dfs_rec( tmp, new_ntk, old2new, old_child1.index );
    mig_man_dup_dfs_rec( tmp, new_ntk, old2new, old_child2.index );
    auto new_child0 = old2new[old_child0.index] ^ old_child0.complement;
    auto new_child1 = old2new[old_child1.index] ^ old_child1.complement;
    auto new_child2 = old2new[old_child2.index] ^ old_child2.complement;

    signal new_node = new_ntk.create_maj( new_child0, new_child1, new_child2 );
    if ( new_equiv != MIG_SIGNAL_NULL && new_equiv.index < new_node.index )
    {
...
template<class Ntk, class RewritingFn, class Library, class RefactoringFn,
         class RefactoringFn2, class NodeCostFn = unit_cost<Ntk>>

Ntk mig_choice_compute( Ntk& ntk, RewritingFn&& rewriting_fn, Library&& library,
                        RefactoringFn&& refactoring_fn,
                        RefactoringFn2&& refactoring_fn2,
                        mig_choice_compute_params const& ps = {},
                        mig_choice_compute_stats* pst = nullptr,
                        NodeCostFn const& cost_fn = {} )
{
  mig_choice_compute_stats st;
  const auto res = [&]()
  {
    return also::detail::mig_choice_compute_manager<
               Ntk, RewritingFn, Library, RefactoringFn, RefactoringFn2,
               NodeCostFn>( ntk, rewriting_fn, library, refactoring_fn,
