#ifndef XMG_CHOICE_COMPUTE_HPP
#define XMG_CHOICE_COMPUTE_HPP
#pragma once
...
namespace also
{
struct xmg_choice_compute_params
{
  xmg_choice_compute_params()
  {
...
struct xmg_choice_compute_stats
{
};
namespace detail
{
struct xmg_cut_enumeration_choice_compute_cut
{
  int32_t gain{ -1 };
};
...
class xmg_choice_compute_manager
{
public:
  using node = typename Ntk::node;
  using signal = typename Ntk::signal;
  static constexpr node XMG_NULL = xmg_network::XMG_NULL;
  static constexpr uint32_t num_vars = 4u;

#define XMG_SIGNAL_NULL ( signal( XMG_NULL, 0 ) )
  xmg_choice_compute_manager( Ntk& ntk, RewritingFn&& rewriting_fn,
  ...
  {
    const auto cuts =
        cut_enumeration<Ntk, true, xmg_cut_enumeration_choice_compute_cut>(
...
private:
  ...
  Ntk derive_choice_xmg()
  {
    ntk.clear_values();
    ntk.init_choices( ntk.size() );
    Ntk choice_xmg;
    choice_xmg.clear_values();
    choice_xmg.init_choices( ntk.size() );
    std::vector<signal> old2new( ntk.size(), XMG_SIGNAL_NULL );
    old2new[0] = ntk.get_constant( false );
    ntk.foreach_pi( [&]( node const& i )
                    {
      choice_xmg.create_pi();
      old2new[i] = signal(i, 0); } );
    auto get_repr_signal = [&]( signal const& s )
    {
      auto repr = choice_xmg.get_repr( s.index );
      return signal( repr, choice_xmg.phase( repr ) ^ choice_xmg.phase( s.index ) ^
                               s.complement );
    };
    ntk.foreach_gate( [&]( node const& n )
      ...
      if (ntk.is_maj(n)) {
        auto new_node = choice_xmg.create_maj(get_repr_signal(new_child0),
        ...
        assert(old2new[n] == XMG_SIGNAL_NULL);
        old2new[n] = new_node;
        ...
        if (choice_xmg.fanout_size(new_node.index) != 0) return;
        choice_xmg.set_choice(new_node.index, new_repr.index);

      } else {
        auto new_node = choice_xmg.create_xor3(get_repr_signal(new_child0),
                                               get_repr_signal(new_child1),
                                               get_repr_signal(new_child2));
        while (true) {
          auto new2 = new_node;
    ...
    ntk.foreach_co( [&]( signal const& o )
      ...
      auto new_repr = choice_xmg.get_repr(new_signal.index);
      choice_xmg.create_po(signal(new_repr, choice_xmg.phase(new_signal.index) ^
                                                choice_xmg.phase(new_repr) ^
                                                new_signal.complement)); } );
    return xmg_man_dup_dfs( choice_xmg );
  }

  Ntk xmg_man_dup_dfs( Ntk const& tmp )
  {
    Ntk new_ntk;
    new_ntk.clear_values();
    new_ntk.init_choices( tmp.size() );
    std::vector<signal> old2new( tmp.size(), XMG_SIGNAL_NULL );
    ...
    tmp.foreach_co( [&]( signal const& o )
                    {
      xmg_man_dup_dfs_rec(tmp, new_ntk, old2new, o.index);
      new_ntk.create_po(old2new[o.index] ^ o.complement); } );
    return new_ntk;
  }

  static signal xmg_man_dup_dfs_rec( Ntk const& tmp, Ntk& new_ntk,
                                     std::vector<signal>& old2new,
                                     node const& n )
  {
    if ( old2new[n] != XMG_SIGNAL_NULL )
      return old2new[n];
    signal new_equiv = signal( XMG_NULL, 0 );
    if ( tmp.get_equiv_node( n ) != XMG_NULL )
    {
      new_equiv =
          xmg_man_dup_dfs_rec( tmp, new_ntk, old2new, tmp.get_equiv_node( n ) );
    }
    auto old_child0 = tmp.get_child0( n );
    auto old_child1 = tmp.get_child1( n );
    auto old_child2 = tmp.get_child2( n );
    xmg_man_dup_dfs_rec( tmp, new_ntk, old2new, old_child0.index );
    xmg_man_dup_dfs_rec( tmp, new_ntk, old2new, old_child1.index );
    xmg_man_dup_dfs_rec( tmp, new_ntk, old2new, old_child2.index );
    auto new_child0 = old2new[old_child0.index] ^ old_child0.complement;
    auto new_child1 = old2new[old_child1.index] ^ old_child1.complement;
    auto new_child2 = old2new[old_child2.index] ^ old_child2.complement;
    if ( tmp.is_maj( n ) )
    {
      ...
      if ( new_equiv != XMG_SIGNAL_NULL && new_equiv.index < new_node.index )
      {
    ...
    {
      signal new_node = new_ntk.create_xor3( new_child0, new_child1, new_child2 );
      if ( new_equiv != XMG_SIGNAL_NULL && new_equiv.index < new_node.index )
      {
        new_ntk.set_equiv( new_node.index, new_equiv.index );
      }
...
Ntk xmg_choice_compute( Ntk& ntk, RewritingFn&& rewriting_fn,
                        RewritingFn2&& rewriting_fn2, Library&& library,
                        RefactoringFn&& refactoring_fn,
                        RefactoringFn2&& refactoring_fn2,
                        xmg_choice_compute_params const& ps = {},
                        xmg_choice_compute_stats* pst = nullptr,
                        NodeCostFn const& cost_fn = {} )
{
  xmg_choice_compute_stats st;
  const auto res = [&]()
  {
    return also::detail::xmg_choice_compute_manager<
