#ifndef AIG_CHOICE_COMPUTE_HPP
#define AIG_CHOICE_COMPUTE_HPP
#pragma once
...
namespace also
{
struct aig_choice_compute_params
{
  aig_choice_compute_params()
  {
...
{
  stopwatch<>::duration time_copy{ 0 };

  void report() const
  {
    fmt::print( "[i] copy node time = {:>5.2f} secs\n", to_seconds( time_copy ) );
  }
};
namespace detail
{
...
struct has_refactoring_with_dont_cares<
    Ntk, RefactoringFn, Iterator,
    std::void_t<decltype( std::declval<RefactoringFn>()(
        std::declval<Ntk&>(), std::declval<kitty::dynamic_truth_table>(),
        std::declval<kitty::dynamic_truth_table>(),
        std::declval<Iterator const&>(), std::declval<Iterator const&>(),
        std::declval<void( Ntk::signal )>() ) )>> : std::true_type
{
};

template<class Ntk, class RefactoringFn, class Iterator>
inline constexpr bool has_refactoring_with_dont_cares_v =
    has_refactoring_with_dont_cares<Ntk, RefactoringFn, Iterator>::value;
struct cut_enumeration_choice_compute_cut
{
  int32_t gain{ -1 };
};
...
class aig_choice_compute_manager
{
public:
...
#define AIG_SIGNAL_NULL ( signal( AIG_NULL, 0 ) )

  aig_choice_compute_manager( Ntk& ntk, RewritingFn&& rewriting_fn,
                              RewritingFn2&& rewriting_fn2, Library&& library,
                              RefactoringFn&& refactoring_fn,
                              RefactoringFn2&& refactoring_fn2,
                              aig_choice_compute_params const& ps,
                              aig_choice_compute_stats& st,
...
private:
  ...
  // int32_t measure_mffc_ref(node const& n,
  //                          cut_enumeration_choice_compute_cut const* cut) {
  ...
  Ntk derive_choice_aig()
  {
    ...
    ntk.foreach_gate( [&]( node const& n )
                      {
      auto repr = ntk.get_choice_representative(n);
      ...
      assert(old2new[n] == AIG_SIGNAL_NULL);
      old2new[n] = new_node;
      ...
      if (choice_aig.fanout_size(new_node.index) != 0) return;
    ...
    return man_dup_dfs( choice_aig );
  }

  Ntk man_dup_dfs( Ntk const& tmp )
  {
    Ntk new_ntk;
    ...
    tmp.foreach_co( [&]( signal const& o )
                    {
      man_dup_dfs_rec(tmp, new_ntk, old2new, o.index);
      new_ntk.create_po(old2new[o.index] ^ o.complement); } );
    return new_ntk;
  }

  static signal man_dup_dfs_rec( Ntk const& tmp, Ntk& new_ntk,
                                 std::vector<signal>& old2new, node const& n )
  {
    if ( old2new[n] != AIG_SIGNAL_NULL )
      return old2new[n];

    signal new_equiv = signal( AIG_NULL, 0 );
    if ( tmp.get_equiv_node( n ) != AIG_NULL )
    {
      new_equiv = man_dup_dfs_rec( tmp, new_ntk, old2new, tmp.get_equiv_node( n ) );
    }
    auto old_child0 = tmp.get_child0( n );
    auto old_child1 = tmp.get_child1( n );

    man_dup_dfs_rec( tmp, new_ntk, old2new, old_child0.index );
    man_dup_dfs_rec( tmp, new_ntk, old2new, old_child1.index );
...
private:
  ...
  std::unordered_map<node, node> forward_map;
}; // namespace detail
} // namespace detail
template<class Ntk, class RewritingFn, class RewritingFn2, class Library,
         class RefactoringFn, class RefactoringFn2,
         class NodeCostFn = unit_cost<Ntk>>
Ntk aig_choice_compute( Ntk& ntk, RewritingFn&& rewriting_fn,
                        RewritingFn2&& rewriting_fn2, Library&& library,
                        RefactoringFn&& refactoring_fn,
                        RefactoringFn2&& refactoring_fn2,
                        aig_choice_compute_params const& ps = {},
                        aig_choice_compute_stats* pst = nullptr,
                        NodeCostFn const& cost_fn = {} )
{
  aig_choice_compute_stats st;
  const auto res = [&]()
  {
    return also::detail::aig_choice_compute_manager<
